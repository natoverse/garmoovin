...
import { createTitleMappingExport, type TitleChange, type TitleMappingExport } from '../src/title-edits'
...
const files: [string, string][] = [
  ['nested/a.gpx', gpx(track('Green Mountain', 'hiking', '2025-01-03T00:00:00Z'))],
  ['other/b.gpx', gpx(track('Green Mountain', 'running', '2025-01-02T00:00:00Z'))],
  ['c.gpx', gpx(track('Riverside Ride', 'cycling', '2025-01-01T00:00:00Z'))],
]
const title = (page: Page, path = 'nested/a.gpx', name = 'Green Mountain') =>
...
const change = (newTitle: string, sourceFile = 'nested/a.gpx', originalTitle = 'Green Mountain'): TitleChange =>
  ({ sourceFile, originalTitle, newTitle })
const mapping = (archive: Buffer, changes: TitleChange[]): TitleMappingExport =>
  ({ schemaVersion: 1, archiveFingerprint: createHash('sha256').update(archive).digest('hex'), changes })
...
test('exports all hidden proposals with exact source paths, original titles, and the ZIP fingerprint', async ({ page }) => {
  ...
  await title(page, 'other/b.gpx').fill('Morning trail run')
  ...
  expect(await save(page)).toEqual(mapping(archive, [
    ...
    change('Morning trail run', 'other/b.gpx'),
  ]))
  ...
  await expect(title(page, 'other/b.gpx')).toHaveValue('Morning trail run')
...
test('repeated saves are complete current snapshots, reuse the hash, and release old download URLs', async ({ page }) => {
  ...
  await title(page, 'other/b.gpx').fill('Second title')
  expect(await save(page)).toEqual(mapping(archive, [change('First title'), change('Second title', 'other/b.gpx')]))
  await title(page).fill('')
  expect(await save(page)).toEqual(mapping(archive, [change('Second title', 'other/b.gpx')]))
...
test('replacement warns before discarding unexported edits and cancellation preserves archive identity', async ({ page }) => {
  ...
  await title(page, 'new.gpx', 'new.gpx').fill('New archive title')
  expect(await save(page)).toEqual(mapping(replacement, [change('New archive title', 'new.gpx', 'new.gpx')]))
})
...
for (const unreadableSibling of [false, true]) {
  test(`duplicate GPX paths block affected proposals even when the other entry is ${unreadableSibling ? 'unreadable' : 'readable'}`, async ({ page }) => {
    const archive = await zip([
      ...
      ['unique.gpx', gpx('<trk><name>Unique route</name></trk>')],
    ])
    let replacements = 0
    ...
    await title(page, 'unique.gpx', 'Unique route').fill('Unambiguous proposal')
    ...
    expect(await save(page)).toEqual(mapping(archive, [change('Unambiguous proposal', 'unique.gpx', 'Unique route')]))
  })
}
