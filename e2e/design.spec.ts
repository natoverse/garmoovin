...
for (const width of [320, 768, 1440]) {
  test(`supplied theme and logo remain usable at ${width}px with CDN fonts unavailable`, async ({ page }) => {
    ...
    const title = page.getByRole('textbox', { name: /^Title for Autumn trail/ })
    await expect(title).toHaveValue('Autumn trail')
    ...
    await expect(page.getByRole('columnheader', { name: 'New title', exact: true })).toHaveCount(0)
    await title.hover()
    await expect(title).not.toHaveCSS('border-top-color', 'rgba(0, 0, 0, 0)')
    await title.focus()
    await expect(title).toHaveCSS('outline-color', 'rgb(95, 107, 51)')
    await expect(title).toHaveCSS('outline-width', '2px')
    await title.fill('Golden hour trail')
