...
test('project-path assets, Google font stylesheet, and private JSON handoff work without a backend', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Title for Original 1 (nested/garmin-1.gpx)', exact: true }).fill('Renamed 1')
  await page.getByRole('textbox', { name: 'Title for Original 2 (garmin-2.gpx)', exact: true }).fill('Renamed 2')
