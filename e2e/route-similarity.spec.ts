...
async function setTolerance(page: Page, value: number) {
  ...
  await expect(tolerance(page)).toHaveAttribute('aria-valuetext', `${value} feet`)
}
...
test('starts off, uses native round feet and keyboard steps, and never chains pair matches into a group', async ({ page }) => {
  await setup(page)
  await expect(grouping(page)).not.toBeChecked()
  await expect(tolerance(page)).toHaveValue('150')
  await expect(tolerance(page)).toHaveAttribute('min', '25')
  await expect(tolerance(page)).toHaveAttribute('max', '1000')
  await expect(tolerance(page)).toHaveAttribute('step', '25')
  await expect(tolerance(page)).toHaveAttribute('aria-valuetext', '150 feet')
  await expect(page.locator('label[for="route-tolerance"]')).toHaveText('Route tolerance: 150 ft — lower is stricter')
  await expect(page.getByRole('combobox', { name: 'Match by' })).toHaveValue('area')
  ...
  await expect(tolerance(page)).toHaveValue('125')
  await expect(tolerance(page)).toHaveAttribute('aria-valuetext', '125 feet')
  await expectGroups(page, files.map(([, contents]) => [/<name>(.*?)<\/name>/.exec(contents)![1]!]))
  await page.keyboard.press('Home')
  await expect(tolerance(page)).toHaveValue('25')
  await expect(tolerance(page)).toHaveAttribute('aria-valuetext', '25 feet')
  await expectGroups(page, files.map(([, contents]) => [/<name>(.*?)<\/name>/.exec(contents)![1]!]))
  await page.keyboard.press('End')
  await expect(tolerance(page)).toHaveValue('1000')
  await expect(tolerance(page)).toHaveAttribute('aria-valuetext', '1000 feet')
  await expectGroups(page, [['Green A', 'Green B', 'Green C'], ['Far away'], ['No geometry']])
...
test('same-area suggestions include route variants and mode changes preserve drafts and reuse analysis', async ({ page }) => {
  const variant = (name: string, length: number, day: number) => gpx(`<trk><name>${name}</name><type>hiking</type><trkseg>
    <trkpt lon="0" lat="0"><time>2025-01-0${day}T00:00:00Z</time></trkpt>
    <trkpt lon="${length * degrees}" lat="0"/>
  </trkseg></trk>`)
  await installProbe(page)
  await setup(page, [
    ['garmin-1.gpx', variant('Full trail', 1000, 3)],
    ['garmin-2.gpx', variant('Shortcut', 800, 2)],
    ['garmin-3.gpx', route('Elsewhere', 1000)],
  ])
  await grouping(page).check()
  await expectGroups(page, [['Full trail', 'Shortcut'], ['Elsewhere']])
  await expect(page.locator('.similarity-panel')).toContainText('a shared center alone is not a match')
  const before = await page.evaluate(() => ({ parses: window.similarityProbe.parses, renders: window.similarityProbe.renders }))
  const title = page.getByRole('textbox', { name: 'Title for Shortcut (garmin-2.gpx)', exact: true })
  await title.fill('Keep this draft')
  await page.getByRole('combobox', { name: 'Match by' }).selectOption('route')
  await expectGroups(page, [['Full trail'], ['Keep this draft'], ['Elsewhere']])
  await expect(page.locator('.similarity-panel')).toContainText('95%')
  ...
  await search(page).fill('shortcut')
  await expectGroups(page, [['Keep this draft']])
...
test('activity cache hits, clearing, and rendering/storage failures preserve matching without network', async ({ page }) => {
  ...
  for (const value of [75, 350, 25, 150]) await setTolerance(page, value)
...
test('superseded group calculations cannot publish stale memberships after slider, filter or archive changes', async ({ page }) => {
  ...
  await setTolerance(page, 650)
  await search(page).fill('green c')
  await setTolerance(page, 25)
  await page.getByRole('combobox', { name: 'Match by' }).selectOption('route')
...
test('several-hundred-route import stays replaceable with unfinished analysis and preserves draft safeguards', async ({ page }) => {
  ...
  await expect(tolerance(page)).toHaveValue('400')
