...
- Prepare matching geometry and thumbnails independently after publishing each activity's metadata. Neither a cached image nor a rendering/storage failure bypasses geometry preparation. Release the raw route after both operations settle.
...
- Keep the existing table and one row per activity. Add a route-suggestion column only while grouping, preserving per-activity keys, preview rendering, title inputs, and unchanged export behavior. Pending and unavailable activities count as ungrouped, not skipped files.
