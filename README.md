...
Activity statistics show **Avg pace** in minutes and seconds per mile for recognized running, walking, and hiking types, or **Avg speed** in mph for cycling types. Averages use recorded horizontal distance and full elapsed time, including pauses, so they may differ from Garmin's moving averages. Pace rounds to the nearest second; speed shows one decimal place. Missing or zero distance/time displays **Unknown**. Elevation is not required; distance never bridges track, segment, or invalid-coordinate gaps. Changing the draft type updates the displayed units; unsupported types omit the average.
...
Each complete activity record contains its starting title/type/date, elapsed duration and recorded distance, route PNG, elevation profile and statistics, and prepared similarity geometry. Titles and types initially come from GPX; **Save JSON** updates only the exported fields in the cache for the next load. A warm hit skips GPX extraction, XML parsing, route hashing/projection, and preview/geometry preparation. Previously computed pair distances are also reused; bundles are still assembled for your current filters and tolerance. New IDs are processed normally. The viewer reports how many entries came **from cache** versus were **processed**.

Older activity records without duration or distance are backfilled from the selected GPX once, counted as processed, without discarding remembered titles/types, existing measurements, or prepared previews and comparisons. Subsequent imports reuse the cached measurements, including **Unknown**.
...
Each **Type** tag is an inline dropdown showing the current type. Click or tab into it to recategorize an activity, for example **Running → Trail Running**; the tag then shows your proposed type. Trail Running is available even if none of your imported activities already use it. The selector offers common Garmin categories plus known types in the current archive; the CLI checks the selected category against Garmin's current catalog before offering a write. Choose the starting type again, or press Escape with the menu closed, to discard just the type draft. Titles and types can be changed independently or together, in ordinary or grouped lists. Type filters use starting types so a row does not disappear while you edit it.

Use each activity's checkbox to select it, including in route bundles. The **Activity selection** header offers **Select all** for currently shown activities and **Select none** to clear the entire selection without discarding edits. Selections survive filtering and regrouping; the header counts any hidden selections. Newly loaded activities are not automatically selected, and opening another archive clears selection.

With two or more activities selected, the header shows shared **Title** and **Type** editors. Common values are displayed; differing values show **Mixed titles** or **Mixed types**. Changes immediately apply to every selected activity, including hidden ones, without changing the other field or unselected rows. Escape restores each selected activity's starting value for that field; leaving a blank title restores starting titles. Individual row edits can override a bulk edit. Selection alone creates no changes; bulk edits use the same Save JSON workflow below.
...
`login` prompts in the terminal for your email, password, and MFA code when required. It saves a private reusable session and changes no activities. `review` loads that session and performs only reads. **`apply` authorizes immediate writes without a confirmation prompt**, including when run without an interactive terminal. It first performs a fresh review and prints the account, original/current/proposed titles and types, source identity, blocking reasons, and eligible count, then executes only eligible changes. Use `review` instead when you only want a preview. Changing the file after review cannot change that invocation's immutable batch.
...
`reconcile` reads the previous journal and Garmin without writing or requiring the original JSON/ZIP. **`reconcile --apply` immediately executes remaining eligible proposals after reconciliation, without prompting.** Already-applied items are not replayed. Unreadable or unsafe journals block the writer; restore them rather than deleting them to bypass recovery. Keep the same journal directory when recovering.

Exit codes: **0** for a fully eligible read-only review or fully confirmed/already-applied result; **1** for blocked/partial results; **2** for input, authentication, storage, or unexpected errors; **130** for interruption. A successful read-only review is not a successful write. CLI results do not synchronize back into the browser, rewrite the input JSON, or alter original titles/GPX files. Reusing an old export rechecks remote state rather than blindly repeating it.
...
Turn on **Group similar routes** to review related routes within the current name and type filters, including across selected activity types. Grouping starts off. **Route tolerance** uses **25–1,000 feet**, in **25-foot steps**, initially **150 ft**; lower is stricter. These are actual feet-based thresholds, not converted labels on meter-based steps. The slider supports arrow keys and Home/End, and assistive technology announces its value in feet.

Matching compares geographic lines, not the independently fitted previews. Location, physical scale, and orientation are preserved; reverse travel and a different start on the same loop can match. **Match by** offers:

- **Similar area** (default): includes strict matches and allows shortcuts or extra distance when at least 70% of each recorded path is within tolerance, the shorter route is at least 60% of the longer, and their length-weighted geographic centers are nearby. Centers must be within the larger of the tolerance or half the smaller route's typical radius (its RMS distance from its center). A shared center alone cannot group unrelated routes.
- **Strict route**: retains the original rule—at least 95% of each recorded path within tolerance and a shorter/longer recorded-length ratio of at least 80%.

Coverage and centers are weighted by route length, not GPS point counts. Similar-area suggestions can include divergent sections or nearby paths; substantially different routes in the same park may remain separate. Review the previews before renaming. The length rules do not relax with the slider.
